...
class OrderBook:
    ...
    def __init__(self, owner, symbol):
        ...
        self.close = deque([100000]*50,maxlen=50)
    def handleLimitOrder(self, order):
        ...
        while matching:
            ...
            if matched_order:
                ...
                self.owner.sendMessage(order.agent_id, Message({"msg": "ORDER_EXECUTED", "order": filled_order, "counter_part" : matched_order.agent_id}))
                self.owner.sendMessage(matched_order.agent_id,
                                       Message({"msg": "ORDER_EXECUTED", "order": matched_order, "counter_part" : order.agent_id}))
        ...
        if not matching:
            ...
            if self.owner.book_freq is not None:
                ...
                for quote, volume, agents in self.getInsideBids():
                    row[quote] = -volume
                    self.quotes_seen.add(quote)
                for quote, volume, agents in self.getInsideAsks():
                    if quote in row:
    ...
    def getInsideBids(self, depth=sys.maxsize):
        ...
        for i in range(min(depth, len(self.bids))):
            ...
            for o in self.bids[i]:
                qty += o.quantity
                agents.append(o.agent_id)
            book.append((price, qty,agents))
    ...
    def prettyPrint(self, silent=False):
        ...
        for quote, volume, agents in self.getInsideAsks()[-1::-1]:
